...
const STATUS_OPTIONS = [
    ...
    { value: 'returning', label: 'Pengembalian' }, { value: 'returned', label: 'Dikembalikan' },
    { value: 'cancelled', label: 'Dibatalkan' },
];

const STATUS_BADGE: Record<string, string> = {
    pending:   'bg-amber-100/80  text-amber-700  dark:bg-amber-900/30  dark:text-amber-400',
    approved:  'bg-blue-100/80   text-blue-700   dark:bg-blue-900/30   dark:text-blue-400',
    rejected:  'bg-red-100/80    text-red-700    dark:bg-red-900/30    dark:text-red-400',
    returning: 'bg-violet-100/80 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400',
    returned:  'bg-green-100/80  text-green-700  dark:bg-green-900/30  dark:text-green-400',
    ...
    return (
        <>
            <tr className="border-b border-border/30 hover:bg-accent/30 transition-colors duration-150 cursor-pointer group" onClick={() => setOpen(o => !o)}>
                ...
                <td className="px-5 py-3.5"><span className={['badge-pill text-[10px]', STATUS_BADGE[row.status] ?? STATUS_BADGE.cancelled].join(' ')}>{row.status_label}</span></td>
                <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                        <span className="text-xs text-muted-foreground">{row.items_count}</span>
                ...
                <tr className="border-b border-border/20 bg-accent/20 animate-fade-in">
                    <td colSpan={7} className="px-8 py-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70 mb-1.5">Tujuan</p>
                                <p className="text-foreground/80">{row.purpose}</p>
                            </div>
                            <div>
                                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70 mb-1.5">Barang</p>
                                <ul className="space-y-0.5">{row.items.map((it, i) => <li key={i} className="text-foreground/80">{it.name} <span className="text-muted-foreground">— {it.quantity} pcs</span></li>)}</ul>
                            </div>
                            {row.approved_by && <div>
                                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70 mb-1.5">Disetujui</p>
                                <p className="text-foreground/80">{row.approved_by} · {row.approved_at}</p>
                            </div>}
                        </div>
                    </td>
                </tr>
        ...
        <div className="space-y-5">
            ...
            <div className="glass-card px-5 py-4 animate-fade-up delay-75">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground/70">Dari Tanggal</label>
                        <input type="date" value={draft.date_from} onChange={e => setDraft(p => ({ ...p, date_from: e.target.value }))} className="input-ios" /></div>
                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground/70">Sampai Tanggal</label>
                        <input type="date" value={draft.date_to} onChange={e => setDraft(p => ({ ...p, date_to: e.target.value }))} className="input-ios" /></div>
                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground/70">Status</label>
                        <select value={draft.status} onChange={e => setDraft(p => ({ ...p, status: e.target.value }))} className="input-ios appearance-none cursor-pointer">
                            {STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
                    <div className="flex items-end gap-2">
                        <Button onClick={apply} className="flex-1 gap-1.5"><Search className="w-3.5 h-3.5" /> Terapkan</Button>
            ...
            <div className="glass rounded-3xl overflow-hidden animate-fade-up delay-150">
                {meta && <div className="px-5 py-3.5 border-b border-border/40"><p className="text-xs text-muted-foreground">Total <span className="font-semibold text-foreground">{meta.total}</span> peminjaman</p></div>}
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead><tr className="border-b border-border/50">
                            {['No','Kode','Peminjam','Tgl Pinjam','Tenggat','Status','Barang'].map((h, i) => (
